...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train glucose response prediction models"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--sample-size", type=int, default=None, help="Limit dataset size for testing"
    )
    ...
    try:
        ...
        if args.sample_size and len(df_fitted_params) > args.sample_size:
            df_fitted_params = df_fitted_params.sample(
                args.sample_size, random_state=42
            )
        ...
        if not args.skip_visualization:
            ...
            plotter.create_comprehensive_report(
                df_fitted_params, model_results, df_ml, xgboost_regressor
            )
        ...
        logger.info(
            f"Dataset: {len(df_fitted_params)} meals from {df_fitted_params['userID'].nunique()} subjects"
        )
        ...
        for target_name, results in model_results.items():
            ...
            logger.info(
                f"  {target_name}: R²={metrics['r_squared']:.3f}, "
