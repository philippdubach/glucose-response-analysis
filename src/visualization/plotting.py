...
class GlucosePlotter:
    ...
    def plot_glucose_curve_fit(self, result: Dict, meal_number: int, save_path: str = None):
        """Plot glucose curve fitting results."""
        fig, ax = plt.subplots(figsize=self.figure_size, dpi=self.dpi)
        
        # Plot data and fit
        ax.scatter(result['t'], result['y_true'], label='Observations', 
                  alpha=0.7, s=30, color='blue')
        ax.plot(result['t'], result['y_fit'], 'r-', label='Fitted Curve', 
               linewidth=2, color='red')
        
        ax.set_xlabel('Time (minutes)', fontsize=12)
        ax.set_ylabel('Glucose Level (mg/dL)', fontsize=12)
        ax.set_title(f'Meal {result["meal"]} - {result["userID"]}: '
                    f'Glucose Response Curve Fitting', fontsize=14)
        ...
        if save_path or self.save_plots:
            if save_path is None:
                ...
                save_path = Path(save_path) / f'glucose_fit_{meal_number}.{self.plot_format}'
            plt.savefig(save_path, dpi=self.dpi, bbox_inches='tight')
        
        return fig, ax
    
    def plot_parameter_distributions(self, df_fitted_params: pd.DataFrame, save_path: str = None):
        ...
        titles = ['Amplitude (A)', 'Time to Peak (δ)', 'Curve Width (σ)', 'Baseline (b)']
        ...
        for i, (param, title, unit) in enumerate(zip(parameters, titles, units)):
            ...
            if param in df_fitted_params.columns:
                ...
                ax.axvline(q25, color='orange', linestyle=':', linewidth=1, label=f'Q25: {q25:.2f}')
                ax.axvline(q75, color='orange', linestyle=':', linewidth=1, label=f'Q75: {q75:.2f}')
        ...
        if save_path or self.save_plots:
            if save_path is None:
                save_path = config.get_results_path('figures') / f'parameter_distributions.{self.plot_format}'
            plt.savefig(save_path, dpi=self.dpi, bbox_inches='tight')
    ...
    def plot_model_performance(self, model_results: Dict, save_path: str = None):
        ...
        fig, axes = plt.subplots(2, n_targets, figsize=(5*n_targets, 10), dpi=self.dpi)
        ...
        for i, target_name in enumerate(targets):
            ...
            ax1.set_title(f'{target_name.title()} - Predictions vs Truth\n'
        ...
        if save_path or self.save_plots:
            if save_path is None:
                save_path = config.get_results_path('figures') / f'model_performance.{self.plot_format}'
            plt.savefig(save_path, dpi=self.dpi, bbox_inches='tight')
        
        return fig, axes
    
    def plot_bland_altman(self, bland_altman_data: Dict, target_name: str, save_path: str = None):
        ...
        # Bias line
        ax.axhline(bias, color='red', linestyle='-', linewidth=2, 
                  label=f'Bias: {bias:.2f}')
        
        # Limits of agreement
        ax.axhline(la95_upper, color='red', linestyle='--', linewidth=1, 
                  label=f'Upper LA: {la95_upper:.2f}')
        ax.axhline(la95_lower, color='red', linestyle='--', linewidth=1, 
        ...
        corr_text = f'Bias correlation: r = {bland_altman_data["bias_correlation"]:.3f}\n'
        ...
        if save_path or self.save_plots:
            if save_path is None:
                save_path = config.get_results_path('figures') / f'bland_altman_{target_name}.{self.plot_format}'
            plt.savefig(save_path, dpi=self.dpi, bbox_inches='tight')
        
        return fig, ax
    
    def plot_shap_values(self, shap_values: np.ndarray, X: pd.DataFrame, 
        ...
        # Summary plot
        shap.summary_plot(shap_values, X, plot_type="bar", show=False, ax=axes[0])
        axes[0].set_title(f'SHAP Feature Importance - {target_name.title()}')
        
        # Beeswarm plot
        shap.summary_plot(shap_values, X, show=False, ax=axes[1])
        axes[1].set_title(f'SHAP Summary Plot - {target_name.title()}')
        
        plt.tight_layout()
        
        if save_path or self.save_plots:
            if save_path is None:
                save_path = config.get_results_path('figures') / f'shap_values_{target_name}.{self.plot_format}'
            plt.savefig(save_path, dpi=self.dpi, bbox_inches='tight')
        
        return fig, axes
    
    def plot_macronutrient_distributions(self, df_ml: pd.DataFrame, save_path: str = None):
        ...
        for i, (macro, title, color) in enumerate(zip(macronutrients, titles, colors)):
            if macro in df_ml.columns:
                ...
                axes[i].axvline(mean_val, color='red', linestyle='--', 
                               linewidth=2, label=f'Mean: {mean_val:.1f}g')
                axes[i].axvline(median_val, color='blue', linestyle=':', 
                               linewidth=2, label=f'Median: {median_val:.1f}g')
        ...
        if save_path or self.save_plots:
            if save_path is None:
                save_path = config.get_results_path('figures') / f'macronutrient_distributions.{self.plot_format}'
            plt.savefig(save_path, dpi=self.dpi, bbox_inches='tight')
        
        return fig, axes
    
    def create_comprehensive_report(self, df_fitted_params: pd.DataFrame, 
                                  model_results: Dict, df_ml: pd.DataFrame,
        ...
        self.plot_parameter_distributions(df_fitted_params, 
                                        figures_path / f'parameter_distributions.{self.plot_format}')
        
        # 2. Macronutrient distributions
        self.plot_macronutrient_distributions(df_ml, 
                                            figures_path / f'macronutrient_distributions.{self.plot_format}')
        
        # 3. Model performance
        self.plot_model_performance(model_results, 
                                   figures_path / f'model_performance.{self.plot_format}')
        ...
        if xgboost_regressor:
            for target_name in model_results.keys():
                ...
                self.plot_bland_altman(bland_altman_data, target_name, 
                                     figures_path / f'bland_altman_{target_name}.{self.plot_format}')
                
                # SHAP plots
                if len(df_ml) > 100:  # Only if we have enough samples
                    X_sample = df_ml.sample(min(100, len(df_ml)))  # Sample for SHAP analysis
                    ...
                    self.plot_shap_values(shap_values, X_features, target_name,
