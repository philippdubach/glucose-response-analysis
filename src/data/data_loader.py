...
class HallDataLoader:
    ...
    def __init__(self):
        self.config = config
        self.hall_data_path = self.config.get_data_path(
            self.config.get("data.hall_data_file")
        )
        self.meals_data_path = self.config.get_data_path(
            self.config.get("data.meals_data_file")
        )
    ...
    def load_hall_data(self) -> pd.DataFrame:
        ...
        logger.info(
            f"Loaded Hall data: {df_hall.shape[0]} records, {df_hall.shape[1]} columns"
        )
        return df_hall
    ...
    def load_meals_data(self) -> pd.DataFrame:
        ...
        df_meals["time"] = pd.to_datetime(df_meals["time"], errors="coerce")
